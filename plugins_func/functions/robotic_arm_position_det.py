...
robotic_arm_pos_function_desc = {
  "type": "function",
  "function": {
    "name": "robotic_arm_position_det",
    "description": "进行机械臂的正运动学(fk)或逆运动学(ik)计算，与robotic_arm_position不同的是，此处的控制是使用相对位置模式",
    "parameters": {
      "type": "object",
      "properties": {
        "mode": {
          "type": "string",
          "enum": ["ik_det", "fk_det"],
          "description": "ik_det 表示相对模式下的逆运动学解算，fk_det 表示相对模式下的正运动学解算"
        },
        "params": {
          "type": "array",
          "items": {
          ...
          "minItems": 3,
          "maxItems": 3,
          "description": "ik_det 模式下填入柱面坐标增量 [r, theta, h]；fk_det 模式下填入机械臂三个关节（底座、肩关节、肘关节）角度增量 [theta1, theta2, theta3]"
        }
      },
...
@register_function(
    "robotic_arm_position_det", robotic_arm_pos_function_desc, ToolType.IOT_CTL
)
async def robotic_arm_position(conn, mode: str, params: list[float]):
    try:
        ...
        # 解析模式
        if mode not in ("ik_det", "fk_det"):
            raise ValueError("mode 参数必须为 'ik_det' 或 'fk_det'")
